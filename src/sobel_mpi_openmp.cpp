...
/**
 * Applies Sobel filter on a local chunk using OpenMP for intra-node parallelism.
 * 
 * - MPI distributes rows across processes (inter-node)
 * - OpenMP parallelizes the row loop within each process (intra-node)
 * 
 * The ext_input buffer includes ghost rows above/below for boundary computation.
 * Only rows [y_start, y_end) in extended coordinates are computed.
 */
static void sobel_local_openmp(const uint8_t* ext_input, uint8_t* ext_output,
                                int width, int ext_rows,
                                int y_start, int y_end)
{
    const int Gx[3][3] = {{-1, 0, 1}, {-2, 0, 2}, {-1, 0, 1}};
    const int Gy[3][3] = {{ 1, 2, 1}, { 0, 0, 0}, {-1,-2,-1}};

    // OpenMP parallelizes rows within each MPI process
    ...
    for (int y = y_start; y < y_end; ++y) {
        for (int x = 1; x < width - 1; ++x) {
            ...
            for (int j = -1; j <= 1; ++j) {
                for (int i = -1; i <= 1; ++i) {
                    int pixel = ext_input[(y + j) * width + (x + i)];
                    sumX += pixel * Gx[j + 1][i + 1];
                    sumY += pixel * Gy[j + 1][i + 1];
                }
            }

            int mag = static_cast<int>(std::sqrt((double)(sumX * sumX + sumY * sumY)));
            ext_output[y * width + x] = static_cast<uint8_t>(std::min(255, mag));
        }
    }
}
...
int main(int argc, char** argv)
{
    ...
    // MPI_THREAD_FUNNELED: only the main thread makes MPI calls (sufficient here)
    int provided;
    MPI_Init_thread(&argc, &argv, MPI_THREAD_FUNNELED, &provided);

    if (provided < MPI_THREAD_FUNNELED) {
        std::cerr << "Warning: MPI does not provide required thread support level.\n";
    }
    ...
    if (argc < 3) {
        if (rank == 0)
            std::cerr << "Usage: mpirun -np <N> " << argv[0]
                      << " <input_image> <output_image> [num_threads_per_proc] [reference_image]\n";
        MPI_Finalize();
        return 1;
    }
    ...
    int num_threads         = (argc >= 4) ? std::stoi(argv[3]) : omp_get_max_threads();
    std::string ref_path    = (argc >= 5) ? argv[4] : "";
    ...
    if (rank == 0) {
        ...
        std::cout << "MPI processes: " << num_procs
                  << " | OpenMP threads per process: " << num_threads
                  << " | Total threads: " << num_procs * num_threads << std::endl;
    }

    // ── Step 2: Broadcast image dimensions ──
    MPI_Bcast(&width,  1, MPI_INT, 0, MPI_COMM_WORLD);
    MPI_Bcast(&height, 1, MPI_INT, 0, MPI_COMM_WORLD);

    // ── Step 3: Row distribution (domain decomposition) ──
    ...
    // ── Step 4: Scatter rows to all processes ──
    ...
    // ── Step 5: Ghost-cell exchange (halo rows for boundary computation) ──
    bool has_top    = (rank > 0);
    ...
    // Send first row up, receive last row from above as ghost_top
    if (has_top) {
        MPI_Sendrecv(local_input.data(),           width, MPI_UNSIGNED_CHAR, rank - 1, 0,
                     ghost_top.data(),             width, MPI_UNSIGNED_CHAR, rank - 1, 1,
                     MPI_COMM_WORLD, MPI_STATUS_IGNORE);
    }

    // Send last row down, receive first row from below as ghost_bottom
    if (has_bottom) {
        MPI_Sendrecv(local_input.data() + (my_rows - 1) * width,
                     width, MPI_UNSIGNED_CHAR, rank + 1, 1,
                     ghost_bottom.data(), width, MPI_UNSIGNED_CHAR, rank + 1, 0,
                     MPI_COMM_WORLD, MPI_STATUS_IGNORE);
    }
    ...
    // ── Step 7: Apply Sobel with OpenMP parallelism ──
    // MPI gave us a chunk of rows; OpenMP threads divide that chunk further
    sobel_local_openmp(ext_input.data(), ext_output.data(),
                       width, ext_rows, 1, ext_rows - 1);

    // Extract this rank's output (strip ghost padding)
    ...
    // ── Step 8: Gather results at rank 0 ──
    ...
    // ── Step 9: Rank 0 saves output and verifies correctness ──
    if (rank == 0) {
        double elapsed_ms = (t_end - t_start) * 1000.0;
        std::cout << "Hybrid MPI+OpenMP Execution time: " << elapsed_ms << " ms" << std::endl;
        ...
        // Optional: compare against serial reference
        if (!ref_path.empty()) {
            int rw, rh, rc;
