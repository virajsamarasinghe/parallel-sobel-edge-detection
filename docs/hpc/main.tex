...
\begin{titlepage}
  \centering
  \IfFileExists{Logo.jpeg}{\includegraphics[width=3cm]{Logo.jpeg}\par\vspace{0.8cm}}{\vspace{0.8cm}}
...
\begin{subfigure}{0.44\textwidth}
  \centering
  \figorbox{lenna_4096.png}{\textwidth}
  \caption{Input: Lenna (4096\,$\times$\,4096 grayscale)}
\end{subfigure}
\hspace{0.06\textwidth}
\begin{subfigure}{0.44\textwidth}
  \centering
  \figorbox{out_serial_4096x4096.png}{\textwidth}
  \caption{Output: Sobel edge map}
\end{subfigure}
\caption{Sobel edge detection on the 4096\,$\times$\,4096 test image.
...
CPU experiments (Serial, OpenMP, Pthreads, MPI, Hybrid) were run on an
Apple Silicon machine (8 physical cores, 8\,GB RAM, macOS), using Open MPI
5.0.9 and Apple Clang with \texttt{-O3}. CUDA experiments were run on a
separate machine with an NVIDIA GeForce GTX\,960M GPU (CUDA 12). Two image
sizes were tested (Table~\ref{tab:datasets}). Each timing measurement was
...
\midrule
Lenna & 4096\,$\times$\,4096 & Standard Lenna test image & 43.45\,ms \\
...
\midrule
Serial        & Lenna, 4K & 1 (baseline) \\
OpenMP        & Lenna, 4K & threads $=$ 1, 2, 4, 8 \\
Pthreads      & Lenna, 4K & threads $=$ 1, 2, 4, 8 \\
MPI           & Lenna, 4K & processes $=$ 1, 2, 4, 8 \\
Hybrid MPI+OMP & Lenna, 4K & (P$\times$T) $=$ 2$\times$2, 2$\times$4, 4$\times$2, 1$\times$8 \\
CUDA          & Lenna, 4K & 16$\times$16 thread blocks (1 thread per pixel) \\
\bottomrule
\end{tabular}
...
\midrule
4096\,$\times$\,4096 & 43.45 & 1.00$\times$ (baseline) \\
...
\toprule
 & \multicolumn{2}{c}{\textbf{Lenna (4096$\times$4096)}} &
   \multicolumn{2}{c}{\textbf{4K (3840$\times$2160)}} \\
\cmidrule(lr){2-3}\cmidrule(lr){4-5}
\textbf{Threads} & Time (ms) & Speedup & Time (ms) & Speedup \\
\midrule
1 & 164.48 & 0.26$\times$ & 61.23 & 0.33$\times$ \\
2 & 95.36  & 0.46$\times$ & 31.87 & 0.63$\times$ \\
4 & 74.24  & 0.59$\times$ & 17.66 & 1.14$\times$ \\
8 & 69.12  & 0.63$\times$ & 13.80 & 1.47$\times$ \\
\bottomrule
\end{tabular}
...
\textbf{Threads} & Time (ms) & Speedup & Time (ms) & Speedup \\
\midrule
1 & 49.28 & 0.88$\times$ & 21.81 & 0.93$\times$ \\
2 & 26.24 & 1.66$\times$ & 11.38 & 1.78$\times$ \\
4 & 16.00 & 2.72$\times$ & 6.67  & 3.03$\times$ \\
8 & 18.56 & 2.34$\times$ & 6.52  & 3.10$\times$ \\
\bottomrule
\end{tabular}
...
\midrule
1 & 15.36 & 1.00$\times$ & 9.38  & 1.00$\times$ \\
2 & 16.64 & 0.92$\times$ & 5.23  & 1.79$\times$ \\
4 & 16.64 & 0.92$\times$ & 4.08  & 2.30$\times$ \\
8 & 30.08 & 0.51$\times$ & 10.33 & 0.91$\times$ \\
\bottomrule
\end{tabular}
...
\midrule
2$\times$2 & 4 & 46.72 & 0.93$\times$ & 18.96 & 1.07$\times$ \\
2$\times$4 & 8 & 39.68 & 1.10$\times$ & 16.33 & 1.24$\times$ \\
4$\times$2 & 8 & 47.36 & 0.92$\times$ & 15.17 & 1.33$\times$ \\
1$\times$8 & 8 & 40.32 & 1.08$\times$ & 17.22 & 1.17$\times$ \\
\bottomrule
\end{tabular}
...
\begin{table}[H]
\centering
\caption{CUDA runtime results (NVIDIA GeForce GTX\,960M). Speedup is relative
  to the CPU serial baseline measured on the same machine (145.23\,ms for
  4096$\times$4096; 71.53\,ms for 4K). Time shown is end-to-end
  (H$\to$D copy + kernel + D$\to$H copy).}
\label{tab:t_cuda}
\begin{tabular}{@{}llllll@{}}
\toprule
\textbf{Image size} & \textbf{Block size} & \textbf{Total blocks} &
\textbf{Kernel (ms)} & \textbf{Total (ms)} & \textbf{Speedup} \\
\midrule
4096$\times$4096      & 16$\times$16 & 256$\times$256 & 1.842 & 9.855  & 14.73$\times$ \\
4K (3840$\times$2160) & 16$\times$16 & 240$\times$135 & 0.964 & 6.289  & 11.37$\times$ \\
\bottomrule
\end{tabular}
\end{table}
...
(Table~\ref{tab:rmse}, Figure~\ref{fig:rmse}). For the CPU implementations
this happens because the Sobel computation uses integer arithmetic and every
pixel is assigned to exactly one thread (no pixel is computed twice). The
CUDA implementation also achieves RMSE\,=\,0 because the kernel uses integer
arithmetic and assigns one thread per pixel with no race conditions.
...
\toprule
\textbf{Version} &
\multicolumn{2}{c}{\textbf{4096$\times$4096}} &
...
\caption{RMSE and PSNR of each parallel version vs the serial reference.
  All versions (including CUDA) achieve perfect accuracy (RMSE\,=\,0,
  PSNR\,=\,$\infty$\,dB).}
\label{tab:rmse}
\end{table}
...
which is large enough to show meaningful scaling behaviour. The 4096$\times$4096
Lenna image results are in Appendix~A (Tables~\ref{tab:t_openmp}--\ref{tab:t_hybrid}).
...
\midrule
CUDA     & 16$\times$16 block & \textbf{6.289} & \textbf{11.37$\times$}\rlap{$^{*}$} \\
\bottomrule
\end{tabular}
\caption{All configurations on the 4K image. Bold = best result per version.
  MPI speedup is internal (relative to MPI-1P); all other CPU speedups are
  relative to the serial baseline of 20.22\,ms (Apple Silicon).
  $^{*}$CUDA speedup is relative to the CPU serial baseline (71.53\,ms)
  measured on the same NVIDIA GTX\,960M machine.}
\label{tab:res_4k}
\end{table}
...
For the 4096$\times$4096 Lenna image (Figure~\ref{fig:speedup_512}), the serial
baseline is 43.45\,ms. Despite the large image, OpenMP on macOS still runs
slower than serial at all thread counts due to the fixed libomp initialisation
overhead---even at this size, the overhead cost exceeds the per-pixel computation
savings. Pthreads, however, achieves meaningful speedup (2.72$\times$ at 4 threads),
demonstrating that the slowdown is specific to the macOS OpenMP runtime, not an
inherent limitation of parallelism for this workload. This teaches us that
\textbf{the right parallel tool matters as much as the parallelism itself}.

\begin{figure}[H]
\centering
\figorbox{speedup_512.png}{0.72\textwidth}
\caption{Speedup vs thread count on the 4096$\times$4096 Lenna image
  (serial baseline: 43.45\,ms). OpenMP libomp initialisation overhead on
  macOS dominates at all thread counts; Pthreads scales well.}
...
simultaneously on an \textbf{NVIDIA GeForce GTX\,960M}. Benchmarks were run with
three averaged trials per image size, covering the full pipeline: Host-to-Device
copy, kernel execution, and Device-to-Host copy.

\begin{itemize}
  \item \textbf{4096$\times$4096 image:} H$\to$D: 4.112\,ms, kernel: 1.842\,ms,
        D$\to$H: 3.901\,ms, \textbf{total: 9.855\,ms}.
        Speedup vs CPU serial (145.23\,ms): \textbf{14.73$\times$} end-to-end,
        \textbf{78.84$\times$} kernel-only.
  \item \textbf{4K (3840$\times$2160) image:} H$\to$D: 2.781\,ms, kernel: 0.964\,ms,
        D$\to$H: 2.544\,ms, \textbf{total: 6.289\,ms}.
        Speedup vs CPU serial (71.53\,ms): \textbf{11.37$\times$} end-to-end,
        \textbf{74.20$\times$} kernel-only.
\end{itemize}

The kernel-only speedups (74--79$\times$) demonstrate the massive parallelism of the
GPU. The end-to-end speedups (11--15$\times$) are lower because PCIe memory transfers
dominate the total time: for the 4K image, the 5.3\,ms of data transfer overhead is
5.5$\times$ larger than the 0.964\,ms kernel itself. This is a classic GPU bottleneck
for memory-bandwidth-limited workloads on systems without unified memory.
The one-time CUDA context initialisation (978\,ms) is excluded from all timings.
...
\caption{Best execution time per implementation on the 4K image. CUDA was
  run on an NVIDIA GeForce GTX\,960M (end-to-end including PCIe transfers).}
\label{fig:compare}
\end{figure}
...
CUDA (GTX\,960M) & 6.289 & 11.37$\times$\rlap{$^{*}$} \\
\bottomrule
\end{tabular}
\caption{Best result per implementation on the 4K image.
  $^{*}$CUDA speedup is vs the CPU serial baseline (71.53\,ms) on the
  same NVIDIA GTX\,960M machine; all other speedups vs Apple Silicon serial (20.22\,ms).}
...
All versions were tested on two image sizes---4096$\times$4096 (Lenna test image)
and 4K (3840$\times$2160)---across 1, 2, 4, and 8 worker configurations. The five
...
\begin{itemize}
  ...
  \item \textbf{Runtime matters as much as image size:} On the 4096$\times$4096
        Lenna image, OpenMP runs \emph{slower} than serial at all thread counts
        on macOS due to the fixed libomp initialisation overhead, even though the
        image is large ($\approx$335\,M operations). Pthreads, by contrast,
        achieves up to 2.72$\times$ speedup at 4 threads on the same image.
        The 4K image (3840$\times$2160, $\approx$165\,M operations) confirms
        that the bottleneck is the OpenMP runtime cost, not insufficient
        workload size.
  ...
  \item \textbf{CUDA achieved the fastest end-to-end times:} On an NVIDIA
        GTX\,960M, the end-to-end time was \textbf{6.289\,ms} for the 4K image
        (11.37$\times$ speedup) and \textbf{9.855\,ms} for the
        4096$\times$4096 image (14.73$\times$ speedup), both with RMSE\,=\,0.
        Kernel-only speedups reached 74--79$\times$; memory transfer overhead
        limits the practical end-to-end gain, but CUDA remains the fastest
        option by a wide margin over all CPU implementations.
\end{itemize}
...
The CPU benchmarks were run on a single Apple Silicon machine (8 cores, macOS)
and the CUDA benchmarks on an NVIDIA GeForce GTX\,960M. Extensions that would
give more complete results include:
\begin{itemize}
  \item Running the Hybrid MPI+OpenMP version on a multi-node Linux HPC cluster,
        where MPI inter-node communication would justify the dual-overhead design.
  \item Testing on a real high-resolution photo (e.g.\ a 4K camera image)
        instead of a synthetic pattern, to validate results on natural image content.
  \item Adding a \textbf{shared memory} optimisation to the CUDA kernel using
        16$\times$16 tiles in \texttt{\_\_shared\_\_} memory to reduce global
        memory reads from 9 to closer to 1 per pixel, further boosting kernel speedup.
  \item Comparing with more image sizes (e.g.\ 1080p, 8K) to draw a full
        scaling curve across all implementations.
\end{itemize}
