...
All parallel versions were tested on two image sizes: a 4096\,$\times$\,4096
Lenna test image and a synthetic 4K image. Every parallel version produced the exact
same output as the serial (single-thread) version, giving RMSE\,=\,0 and
PSNR\,=\,$\infty$\,dB, which means perfect accuracy.

For the 4K image on an 8-core Apple Silicon machine, the best results were:
POSIX Threads with 4 threads gave a \textbf{3.03$\times$} speedup; MPI with
4 processes gave the best raw time of \textbf{4.08\,ms}. The Hybrid MPI+OpenMP
version ran at \textbf{15.17\,ms} with 4 processes and 2 threads each.
OpenMP showed higher overhead on macOS due to the Homebrew libomp runtime.
CUDA results are pending a GPU cluster run; we expect \textbf{10--40$\times$}
speedup based on the massively parallel nature of the GPU kernel
(one thread per pixel on a 16\,$\times$\,16 thread-block grid).
...
\midrule
Lenna & 4096\,$\times$\,4096 & Standard Lenna test image & 43.52\,ms \\
...
\midrule
4096\,$\times$\,4096 & 43.52 & 1.00$\times$ (baseline) \\
...
\midrule
4096$\times$4096 & 16$\times$16 & 256$\times$256  &
  \textit{[Pending]} & \textit{[Pending]} \\
4K (3840$\times$2160) & 16$\times$16 & 240$\times$135 &
  \textit{[Pending]} & \textit{[Pending]} \\
\bottomrule
\end{tabular}
...
CUDA         & \multicolumn{4}{c}{\textit{[Pending GPU cluster run]}} \\
\bottomrule
\end{tabular}
...
\midrule
CUDA     & 16$\times$16 block & \textit{[Pending]} & \textit{[Pending]} \\
\bottomrule
\end{tabular}
...
\begin{itemize}
  ...
  \item \textbf{CUDA (pending):} Based on the GPU's massively parallel
        architecture (one thread per pixel, thousands of CUDA cores), we
        expect the fastest results. Implementation is complete; GPU cluster
        results are pending.
\end{itemize}
